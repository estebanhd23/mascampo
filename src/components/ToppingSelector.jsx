...
export default function ToppingSelector({ current, setCurrent, menu, ackExtras = false, onNeedExtras }) {
  const lista = Array.isArray(menu?.toppings) ? menu.toppings : [];
  const bowl = Array.isArray(menu?.bowls) ? menu.bowls.find((b) => b.id === current?.bowlId) : null;
  const incluidas = Number(bowl?.toppingsIncluidos || 0);

  const totalSeleccionadas = useMemo(
    () => Object.values(current?.toppings || {}).reduce((a, c) => a + c, 0),
    [current?.toppings]
  );
